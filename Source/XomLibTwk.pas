...
const
 APPVER = 'XomLibTwk';
  ...
  FontGL  = 2000;
  MaxDeph = 100000.0;  // ìàêñèìàëüíàÿ ãëóáèíà
...
// íîâûé êîä
      ...
      // îòêðûâàåì ôàéë è çàãðóæàåì â ïàìÿòü.
...
// Ñ÷èòûâàåì êîëè÷åñòâî êîíòåðéíåðîâ
...
try
// Öèêë ïîêîéòåéíåðíîãî ñ÷èòûâàíèÿ Íàçâàíèé êîíòåéíåðîâ
...
//Ñ÷èòûâàíèå ñ ïðîâåðêîé íà÷àëà òàáëèöû
 ...
 // But.Status.Text := Format('Strings: (%d) - (%d)', [MaxInx, LenSTR]);
  ...
  Outpoint := false; // âûëåò èç ïàìÿòè
    ...
    if Size > 0 then
      for i := 1 to Size do
        ...
        begin
          ...
          if (IsCtnr) then  // èùåì êîíåö êîíòåéíåðà
...
begin
  j := LStrings.Count-1; // íóëåâîé èíäåêñ çàíÿò
//  p2 := VBuf;
  p4 := Pointer(Longword(p) + j * 4);  // ðàçìåð òàáëèöû èíäåêñîâ òåêñòà
  Len := 1;
 ...
 //   Smallint(Pointer(i * 4 + Longword(p))^) := Smallint(Len);  // çàïîëíÿåì äëèííû â èíäåêñû
    p3 := Pointer(Longword(p4) + Len);
    s2:= Utf8Encode(s);
   // s2:=s;
    p1  := PChar(s2);
    Move(p1^, p3^, Length(s2));// êîïèðóåì òåêñò â ïÿìÿòü
    Len := Len + Length(s2) + 1;
  end;
  SortStrings.Free;
  Longword(Pointer(Longword(p) - 12)^) := Longword(j + 1); // ïèøåì êîëè÷åñòâî ñëîâ
  Longword(Pointer(Longword(p) - 8)^) := Longword(Len);  // äëèííà ñëîâ
  p := Pointer(Longword(p4) + Len); // ïðûãàåì ïîñëå òàáëèöû ñëîâ
end;
...
begin
 // OldXom := Buf;
  ...
  VBufBegin := AllocMem(1024*1024); // áåðåì ïàìÿòü äëÿ ñòðîê è øàïîê
  VBuf := VBufBegin;
  p2:=VBuf;
